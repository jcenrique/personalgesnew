<div class="space-y-4">
    <div class="flex items-center justify-between">
        <h3 class="text-lg font-semibold">{{ $trainingAction->course->name }}</h3>
        <span class="text-sm text-gray-600">Total: {{ $attendees->count() }}</span>
    </div>

    <div class="space-y-2 text-sm text-gray-600">
        <p>
            <span class="font-semibold">{{ __('Acción formativa:') }}</span>
            {{ $trainingAction->trainer_name }} - {{ $trainingAction->start_date->translatedFormat('d F Y') }}
        </p>
        <p>
            <span class="font-semibold">{{ __('Empresa:') }}</span>
            {{ $trainingAction->company_name }}
        </p>
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm border-collapse">
            <thead>
                <tr class="border-b bg-gray-100 text-gray-800">
