                 ...
                 TextColumn::make('rechazable_type')
                    ...
                    ->formatStateUsing(function ($state) {
                        ...
                        if ($state === 'App\Models\Sabado') {
                            return __('Sábado');
                        }else if ($state === 'App\Models\Additionalday') {
                            return __('Día adicional');
                        }else if ($state === 'App\Models\Computo') {
                            return __('Cómputo');
                        }else if ($state === 'App\Models\Companyday') {
                    ...
                    ->description(function ( $record) {
                        ...
                        }else if ($record->rechazable_type === 'App\Models\Additionalday') {
                            return  $record->rechazable()->latest()->first()->year;
                        }else if ($record->rechazable_type === 'App\Models\Computo') {
                            return  $record->rechazable()->latest()->first()->year;
                        }else if ($record->rechazable_type === 'App\Models\Companyday') {
            ...
            ->filters([
                //filtro por usuario utilizando la relación entre rechazo y usuario para obtener el nombre del usuario asociado al rechazo
                    ...
                    ->options(function () {
                        ...
                        $users = \App\Models\User::whereHas('rechazos')->pluck('name', 'id');
                        return $users;
                    })
                    ->searchable()
